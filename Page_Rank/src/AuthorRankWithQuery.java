...
public class AuthorRankWithQuery {
	...
	public static void main(String[] args) {
		...
		try {
			...
			IndexReader authorIndex = DirectoryReader.open(FSDirectory.open(new File("C:\\Users\\AVIATOR\\Desktop\\IU\\Sem1\\SEARCH\\Assignment\\Assignment 3\\author_index")));
			IndexSearcher searcher = new IndexSearcher(authorIndex);
			...
			for(int i = 0; i <numberOfEdges; i++) {
				...
				graph.addEdge(Integer.toString(i+1),new Pair<String>(vertices.get(splittedLine[0]), vertices.get(splittedLine[1])),EdgeType.DIRECTED);
			}
			bufferedReader.close();
			// Calculating prior of the vertices (i.e. authors) who are relevant to the user query
			Map<String, Double> prior = new HashMap<String, Double>();
			Query q = parser.parse(queryString);
			TopDocs hits =searcher.search(q, 300);
			ScoreDoc[] scoreDocs = hits.scoreDocs;
			Double sumOfPrior = 0.0;
			for(int n=0; n<scoreDocs.length; n++ ) {
				ScoreDoc sd = scoreDocs[n];
				String authorId = searcher.doc(sd.doc).get("authorid");
				sumOfPrior+=sd.score;
				prior.put(authorId, prior.containsKey(authorId) ? prior.get(authorId) + sd.score : sd.score);
			}
			// Normalizing the prior score
			for(String key: prior.keySet() ) {
				prior.put(key,prior.get(key)/sumOfPrior);	
			}
			// Transforming the prior to containing all the vertices which are present in the graph 
			Transformer<String, Double> authorPrior = new Transformer<String, Double>(){
				@Override
				public Double transform(String v) {
					if(prior.containsKey(v)) {
						return  prior.get(v);
					}
					else {
						return 0.0;
			...
			// Creating a page rank object with prior
			PageRankWithPriors<String, String> ranker = new PageRankWithPriors<String, String>(graph ,authorPrior, 0.15);
